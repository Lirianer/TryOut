...
/*
    class WallCell : GridCell
    {
        ...
        public WallCell(GridCell gridCell) : base(gridCell.Rect.Width, gridCell.Rect.X, gridCell.Rect.Y, gridCell.X, gridCell.Y)
        {
            base.X = gridCell.X;
            base.Y = gridCell.Y;
            base.isWall = true;
            size = gridCell.Rect.Width;
            texture = new Bitmap(Resources.wall);
            textureBrush = new TextureBrush(texture);

            rect = new Rectangle(gridCell.Rect.X, gridCell.Rect.Y, size + 1, size);
            imageRectangle = new Rectangle(gridCell.Rect.X, gridCell.Rect.Y, size + 1, size + 1);
        }        
